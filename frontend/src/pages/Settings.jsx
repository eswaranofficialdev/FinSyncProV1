...
const Settings = () => {
  ...
  useEffect(() => {
    ...
    // Check if global pre-captured prompt exists
    ...
    const handleAppInstalled = () => {
      ...
      toast.success('FinSync installed successfully!');
    };
  ...
  const handleInstallClick = async (event) => {
    ...
    if (isInstalled) {
      toast.info('FinSync is already installed.');
      return;
    }
    ...
    if (activePrompt) {
      try {
        ...
        toast.error('Unable to start the installation.');
      } finally {
        setDeferredPrompt(null);
        window.deferredPrompt = null;
      }
    ...
    toast.info(
      'To install FinSync: Tap your browser menu (three dots in the top-right corner) and select "Install app" or "Add to Home screen".',
      { autoClose: 7000 }
    );
  };
          ...
          <form onSubmit={prefsForm.handleSubmit(savePrefs)}>
            ...
            <div className="settings-section">
              ...
              <div className="mode-selector">
                ...
                >
                  <span className="mode-icon"><FaSun /></span>
                  <span>
                    <strong>Light</strong>
                    <small>Bright interface</small>
                  </span>
                  {theme === 'light' && (
                    <span className="mode-check"><FaCheck /></span>
                  )}
                </button>
            ...
            <div className="settings-section">
              <div className="settings-section-title">
                <div>
                  <h2>Color Theme</h2>
                  <p>Personalize your FinSync dashboard</p>
                </div>
                <span className="current-theme-label">
                  {colorThemes.find((item) => item.id === colorTheme)?.name}
                </span>
              </div>

              <div className="color-theme-grid">
                {colorThemes.map((item) => {
                  const selected = colorTheme === item.id;
                  return (
                    <button
                      key={item.id}
                      type="button"
                      className={`color-theme-option ${selected ? 'active' : ''}`}
                    ...
                    >
                      <span
                        ...
                        style={{
                          background: `linear-gradient(135deg, ${item.color}, ${item.secondary})`,
                        }}
                      />
            ...
            <div className="settings-section">
              ...
              >
                ...
                {!isInstalled && (
                  <button type="button" className="btn btn-outline" onClick={handleInstallClick}>
